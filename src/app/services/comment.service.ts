...
export class CommentService {
  ...
  getAllCommentsForNews(newsId: string): Observable<Comments[]> {
    const comments = this.db
      .collection<Comments>("comments", (ref) =>
        ref.where("newsId", "==", newsId).orderBy("commentDate", "desc")
      )
      .snapshotChanges()
      .pipe(
        map((actions) => {
  ...
  deleteAllCommentForNews(newsId: string) {
    const commentsToDelete = this.db
      .collection("comments", (ref) => ref.where("newsId", "==", newsId))
      .snapshotChanges();
